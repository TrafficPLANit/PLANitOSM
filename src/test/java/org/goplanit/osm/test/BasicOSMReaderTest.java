...
public class BasicOSMReaderTest {
  ...
  public void osmReaderRoadInfrastructureTest() {
    try {
      ...
      // are deemed correct
      assertEquals(network.getTransportLayers().size(), equalTo(1));
      assertEquals(network.getTransportLayers().getFirst().getLinks().size(), equalTo(1075));
      assertEquals(network.getTransportLayers().getFirst().getLinkSegments().size(), equalTo(2119));
      assertEquals(network.getTransportLayers().getFirst().getNodes().size(), equalTo(882));
