...
public class OsmIntermodalReaderFactory {
  ...
  /** Create a PLANitOsmIntermodalReader which will create its own macroscopic network, zoning and non-locale
   * specific defaults for any right hand driving country
   * 
  ...
  /** Create a PLANitOsmIntermodalReader which will create its own macroscopic network, zoning. Locale based
   * on country name provided
   * 
   ...
   * 
   * @param inputFile to use
   * @param countryName country which the input file represents, used to determine defaults in case
   *                   not specifically specified in OSM data, when left blank global defaults will be used
   * based on a right hand driving approach
  ...
  public static OsmIntermodalReader create(URL inputSource, String countryName) {
    ...
    return create(
        ...
        countryName,
        networkToPopulate,
        new Zoning(networkToPopulate.getIdGroupingToken(), networkToPopulate.getNetworkGroupingTokenId()));
  }  
  ...
  /** Create a PLANitOsmIntermodalReader which requires the user to set the remaining required settings on
   *  the provided settings instances
   * 
  ...
  public static OsmIntermodalReader create(OsmIntermodalReaderSettings settings) {
    ...
    networkToPopulate.setXmlId(networkToPopulate.getId());
    var zoningToPopulate = new Zoning(
        networkToPopulate.getIdGroupingToken(),  networkToPopulate.getNetworkGroupingTokenId());
