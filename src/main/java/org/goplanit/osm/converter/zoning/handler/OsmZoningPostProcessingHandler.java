...
/**
 * Handler that conducts final parsing round where all stop_positions in relations are mapped to the now parsed
 * transfer zones. This is done separately because transfer zones are sometimes also part of relations, and it is
 * not guaranteed that all transfer zones are available when encountering a stop_position in a relation. So we
 * parse them in another pass.
 * <p>
 * Also, all unprocessed stations that are not part of any relation are converted into transfer zones and
 * transfer connectoids here since
...
public class OsmZoningPostProcessingHandler extends OsmZoningHandlerBase {
  ...
  /** to be able to find OSM nodes internal to parsed PLANit links that we want to break to for example create stop
   * locations for stand alone station, we must be able to spatially find those nodes spatially because they are not
   * referenced by the station or PLANit link explicitly, this is what we do here. It is not placed in the zoning
   * data as it is only utilised in post-processing */
  private Map<MacroscopicNetworkLayer, Quadtree> spatiallyIndexedOsmNodesInternalToPlanitLinks = null;

  /** A stand alone station can either support a single platform when it is road based or two stop_locations for
   * rail (on either side). This is reflected in the returned max matches. The search distance is based on the
   * settings where a road based station utilises the stop to waiting
  ...
  private static Pair<Double, Integer> determineSearchDistanceAndMaxStopLocationMatchesForStandAloneStation(
    ...
    if(OsmRailModeTags.isRailModeTag(osmStationMode)) {
      ...
      LOGGER.warning(String.format("DISCARD: water based stand-alone station detected %d, not supported yet, skip",
          osmStationId));
      return null;
    }
  ...
  private void initialiseSpatiallyIndexedOsmNodesInternalToPlanitLinks() {
    ...
    for(MacroscopicNetworkLayer layer : getReferenceNetwork().getTransportLayers()) {
      ...
      for( Point location: registeredInternalLinkLocations) {
        ...
        getGeoUtils().createBoundingBox(pointEnvelope, 5); // buffer around bounding box of point to
        // avoid issues with JTS quadtree minimumExtent anomalies
  ...
  /** From the provided options, select the most appropriate based on proximity, mode compatibility, relative location
   * to transfer zone, and importance of the OSM way type
   *
   ...
   */
  private Pair<MacroscopicLink, Set<LinkSegment>> findMostAppropriateStopLocationLinkForWaitingArea(
      TransferZone transferZone, String osmAccessMode, Collection<MacroscopicLink> eligibleLinks) {
    ...
    /* 1) reduce candidates to access links related to access link segments that are deemed valid in terms of mode
    and location (closest already complies as per above) */
    ...
    var candidatesToFilter =
        accessLinkSegments.stream().flatMap(
            ls -> Stream.of((MacroscopicLink)ls.getParent())).collect(Collectors.toSet());

    /* 2) make sure a valid stop_location on each remaining link can be created (for example if stop_location would be
     * on an extreme node, it is possible no access link segment upstream of that node remains
    ...
    for(var candidate : candidatesToFilter){
      if(null != ZoningConverterUtils.findTransferConnectoidLocationForWaitingAreaOnLink(
          transferZone.getExternalId(),
          transferZone.getGeometry(),
          candidate,
    ...
    /* 3) all proper candidates so  reduce options further based on proximity to closest viable link, while removing
     * options outside of the closest distance buffer */
    var filteredCandidates =
        PlanitGraphGeoUtils.findEdgesWithinClosestDistanceDeltaToGeometry(
            transferZone.getGeometry(),
            candidatesWithValidConnectoidLocation,
            OsmPublicTransportReaderSettings.DEFAULT_CLOSEST_EDGE_SEARCH_BUFFER_DISTANCE_M,
            getGeoUtils()).keySet();
    ...
    if(!(accessMode.getPhysicalFeatures().getTrackType() == TrackModeType.RAIL) &&
      ...
      var lowerCapacitySegments =
          filteredCandidates.stream().flatMap(l -> l.getLinkSegments().stream()).filter(
              ls -> Precision.smaller(ls.getCapacityOrDefaultPcuHLane(), maxCapacity.get(), Precision.EPSILON_6)).
              collect(Collectors.toUnmodifiableSet());
      accessLinkSegments.removeAll(lowerCapacitySegments);
      filteredCandidates.removeAll(
          lowerCapacitySegments.stream().map(MacroscopicLinkSegment::getParent).collect(
  ...
  /** Find all links that are within the given search bounding box, are mode compatible, and have a matching
   * vertical layer index, i.e., reside on the same vertical plane (if the zone has a known layer)
   *
   ...
   */
  private Map<MacroscopicNetworkLayer, Collection<MacroscopicLink>>
  findModeBBoxVerticalLayerIdxCompatibleLinksForTransferZone(
    ...
    Map<MacroscopicNetworkLayer, Collection<MacroscopicLink>> spatiallyMatchedLinksByLayer =
        getZoningReaderData().getPlanitConverterData().findLinksSpatially(searchBoundingBox);
    if(spatiallyMatchedLinksByLayer == null || spatiallyMatchedLinksByLayer.isEmpty()) {
      return null;
    }    
  
    /* filter based on mode compatibility */
    var modeAndSpatiallyCompatibleLinksByLayer = getPtModeHelper().filterModeCompatibleLinks(
        eligibleOsmModes, spatiallyMatchedLinksByLayer, false /*only exact matches allowed */);
    if(CollectionUtils.nullOrEmpty(modeAndSpatiallyCompatibleLinksByLayer.values())) {
      return null;
    }

    /* filter based on vertical layer index compatibility */
    var modeSpatiallyAndVerticalPlaneCompatibleLinks =
        getTransferZoneHelper().filterVerticalLayerCompatibleLinks(
            transferZone, modeAndSpatiallyCompatibleLinksByLayer, true);
    if(CollectionUtils.nullOrEmpty(modeSpatiallyAndVerticalPlaneCompatibleLinks.values())) {
      return null;
    }
  ...
  /** find links that are within the given search bounding box and are mode compatible with the given reference modes.
   * If more links are found than maxMatches, reduce the results to the closest maxMatches. We also make sure that in
   * case multiple matches are allowed we only select multiple links if they represent parallel train lines.
   * 
  ...
  private TreeSet<MacroscopicLink> findStopLocationLinksForStation(
      OsmEntity stationEntity, TransferZone transferZone, String referenceOsmMode, Envelope searchBoundingBox,
      Integer maxMatches){

    Map<MacroscopicNetworkLayer, Collection<MacroscopicLink>> dirModeSpatiallyCompatibleLinksByLayer =
        findModeBBoxVerticalLayerIdxCompatibleLinksForTransferZone(
            transferZone, stationEntity.getId(), referenceOsmMode, searchBoundingBox);
    if(dirModeSpatiallyCompatibleLinksByLayer == null || dirModeSpatiallyCompatibleLinksByLayer.isEmpty()){
      return null;
    }

    var flattenedDirModeSpatiallyCompatibleLinks = dirModeSpatiallyCompatibleLinksByLayer.size() == 1 ?
        dirModeSpatiallyCompatibleLinksByLayer.values().iterator().next() :
        dirModeSpatiallyCompatibleLinksByLayer.values().stream().flatMap(
            Collection::stream).collect(Collectors.toList());

    /* #matches compatibility */
    TreeSet<MacroscopicLink> chosenLinksForStopLocations = new TreeSet<>();
    {
      var idealAccessResult = findMostAppropriateStopLocationLinkForWaitingArea(
          transferZone, referenceOsmMode, flattenedDirModeSpatiallyCompatibleLinks);
      var idealAccessLink = idealAccessResult==null ? null : idealAccessResult.first();
      if(idealAccessLink==null) {
        LOGGER.severe(String.format(
                "DISCARD No appropriate link could be found from selection of eligible closeby OSM ways [%s] when " +
                    "finding stop locations for station %s, this should not happen, as a workaround, consider excluding" +
                    " this stop",
            flattenedDirModeSpatiallyCompatibleLinks.stream().map(ExternalIdAble::getExternalId).collect(
                Collectors.joining(",")), transferZone.getExternalId()));
        return null;
      }
      ...
      if(maxMatches==1) {
        ...
        /* multiple matches allowed, indicating we are searching for parallel platforms -> use closest match to
         * establish a virtual line from station to this link's closest intersection point
         * and use it to identify other links that intersect with this virtual line, these are our parallel platforms */
        LineSegment stationToClosestPointOnClosestLinkSegment;
        ...
        if(Osm4JUtils.getEntityType(stationEntity) == EntityType.Node) {
          Coordinate closestCoordinate = OsmNodeUtils.findClosestProjectedCoordinateTo(
              (OsmNode)stationEntity, idealAccessLink.getGeometry(), getGeoUtils());
          Point osmStationLocation = PlanitJtsUtils.createPoint(OsmNodeUtils.createCoordinate((OsmNode)stationEntity));
          stationToClosestPointOnClosestLinkSegment = PlanitJtsUtils.createLineSegment(
              osmStationLocation.getCoordinate(),closestCoordinate);
          ...
          stationToClosestPointOnClosestLinkSegment = OsmWayUtils.findMinimumLineSegmentBetween(
                  (OsmWay)stationEntity, idealAccessLink.getGeometry(),
              getZoningReaderData().getOsmConverterData().getOsmNodeData().getRegisteredOsmNodes(), getGeoUtils());
        }else {
          throw new PlanItRunTimeException("Unknown entity type %s for osm station encountered, this should " +
              "not happen", Osm4JUtils.getEntityType(stationEntity));
        }

        final LineSegment interSectionLineSegment = getGeoUtils().createExtendedLineSegment(
            stationToClosestPointOnClosestLinkSegment,
            getSettings().getStationToParallelTracksSearchRadiusMeters(),
            true,
            true);
        final Geometry virtualInterSectionGeometryForParallelTracks =
            PlanitJtsUtils.createLineString(
                interSectionLineSegment.getCoordinate(0),interSectionLineSegment.getCoordinate(1));
        
        /* find all links of compatible modes that intersect with virtual line reflecting parallel accessible
        (train) tracks eligible to create a platform for */
        for(var link : flattenedDirModeSpatiallyCompatibleLinks) {
          if( link.getGeometry().intersects(virtualInterSectionGeometryForParallelTracks)) {
            /* intersect so still possible */
            final LinearLocation closestLinkLinearLocation =
                getGeoUtils().getClosestGeometryExistingCoordinateToProjectedLinearLocationOnLineString(
                    transferZone.getGeometry(), link.getGeometry());
            final var closestLinkLocation = closestLinkLinearLocation.getCoordinate(link.getGeometry());
            final double distanceStationToPotentialAccessLink =
                getGeoUtils().getClosestDistanceInMeters(closestLinkLocation, transferZone.getGeometry());
    ...
    if(chosenLinksForStopLocations.isEmpty()) {
      /* cannot happen because at least the closestLinkForStopLocation we know exists should be found here */
      throw new PlanItRunTimeException("No links could be identified from virtual line connecting station to " +
          "closest by point on closest link for osm station %d, this should not happen", stationEntity.getId());
    }
  ...
  /**
   * Try to extract a station from the entity. In case no existing platforms/stop_positions can be found nearby we
   * create them fro this station because the station
   * represents both platform(s), station, and potentially stop_positions. In case existing platforms/stop_positions
   * can be found, it is likely the station is a legacy
   * tag that is not yet properly added to an existing stop_area, or the tagging itself only provides platforms
   * without a stop_area. Either way, in this case the station is
   ...
   * @param eligibleSearchBoundingBox the search area to see if more detailed and related existing infrastructure
   *                                  can be found that is expected to be conected to the station
   * @param suppressWarnings          suppress warnings when true
   */
  private void processLandBasedStationNotPartOfStopArea(
      OsmEntity osmStation,
      Collection<String> eligibleOsmModes,
      Envelope eligibleSearchBoundingBox,
    ...
    Collection<TransferZone> potentialTransferZones =
        getZoningReaderData().getPlanitConverterData().getTransferZonesSpatially(eligibleSearchBoundingBox);
    if(potentialTransferZones != null && !potentialTransferZones.isEmpty()) {          
      ...
      for groups with multiple members we enforce exact mode compatibility and do not allow for pseudo
      compatibility (yet) */
      Set<TransferZoneGroup> potentialTransferZoneGroups =
          getTransferZoneGroupHelper().findModeCompatibleTransferZoneGroups(
              eligibleOsmModes, potentialTransferZones, false /* exact mode compatibility */);
      if(potentialTransferZoneGroups!=null && !potentialTransferZoneGroups.isEmpty()) {
        ...
        TransferZone closestZone = PlanitTransferZoneUtils.findTransferZoneClosestByTransferGroup(
            ...
            getZoningReaderData().getOsmConverterData().getOsmNodeData().getRegisteredOsmNodes(),
            suppressWarnings,
            getGeoUtils());
        ...
        groups.stream().sorted(Comparator.comparing(TransferZoneGroup::getId)).forEach( group -> {
          ...
          for(TransferZone zone : group.getTransferZones()) {
            PlanitTransferZoneUtils.updateTransferZoneOverArchingNameIfAbsent(zone, tags);
        ...
        /* try finding stand-alone transfer zones that are pseudo mode compatible instead, i.e., we are less strict
        at this point */
        // todo consider modeless transferzone matches if we do not find a match but there exist a modeless
        //  transferzone we should find (not yet supported)
    ...
    if(matchedTransferZones.isEmpty()) {
      ...
      if(!eligibleOsmModes.isEmpty()) {
         ...
         * */
        extractStandAloneStation(osmStation, tags, suppressWarnings, getGeoUtils());
      }             
    }else if(LOGGER.getLevel() == Level.FINE){            
      String transferZonesExternalId = matchedTransferZones.stream().map(ExternalIdAble::getExternalId).collect(
  ...
  /**
   * process remaining unprocessed stations of a particular type that are not part of any stop_area. This means the
   * station reflects both a transfer zone and an implicit stop_position at the nearest viable node if it cannot be
   * mapped to a nearby platform/stop_area
   *
   * @param unprocessedStations stations to process
   * @param type of the stations
   */  
  private void processStationsNotPartOfStopArea(
      Set<OsmEntity> unprocessedStations, EntityType type, OsmPtVersionScheme ptVersion) {
    ...
    if(unprocessedStations != null) {
      unprocessedStations.stream().sorted(Comparator.comparing(OsmEntity::getId)).forEach( osmStation ->{
        ...
        boolean outsideBoundingArea = !getProjectedBoundingAreaHelper().fallsWithinSpatiallyEligibleBoundingArea(
            osmStation, type, tags, getZoningReaderData().getOsmConverterData().getOsmNodeData());
        ...
        /* regular station processing */
          ...
          Envelope boundingBox = OsmBoundingAreaUtils.createBoundingBoxForOsmEntity(
              osmStation,
          ...
          if (boundingBox != null) {
            ...
            processLandBasedStationNotPartOfStopArea(osmStation, eligibleOsmModes, boundingBox, suppressWarnings);
            ...
            switch (ptVersion) {
              ...
              case VERSION_2:
                ...
                break;
              default:
                LOGGER.severe(String.format("Unknown Pt version found %s when processing station %s not part of a " +
                    "stop_area", osmStation.getId(), ptVersion.toString()));
                break;
            }
  ...
  /**
   * process any remaining unprocessed stations that are not part of any stop_area. This means the station reflects
   * both a transfer zone and an implicit stop_position at the nearest viable node
   *
   */
  private void processStationsNotPartOfStopArea() {
    OsmZoningReaderOsmData osmData = getZoningReaderData().getOsmConverterData();
  ...
  /**
   * If ferry terminal is mapped to another user defined waiting area, detect and retrieve it
   * @param osmFerryTerminal to check
   * @param tags to use
   * @return transfer zone if any is user mapped, null otherwise
   */
  public TransferZone findUserOverwrittenWaitingAreaForFerryTerminal(
      OsmEntity osmFerryTerminal, Map<String, String> tags){
    if(getSettings().isOverwriteWaitingAreaOfStopLocation(osmFerryTerminal.getId())) {
      /* transfer zone to use is user replaced, so immediately adopt this transfer zone  */
      Pair<EntityType, Long> result = getSettings().getOverwrittenWaitingAreaOfStopLocation(osmFerryTerminal.getId());
      var ferryTerminalTransferZone = getZoningReaderData().getPlanitConverterData().getTransferZoneByOsmId(
          result.first(), result.second());
      LOGGER.fine(String.format("Mapped ferry stop %d to overwritten waiting area %d",
          osmFerryTerminal.getId(), result.second()));
      PlanitTransferZoneUtils.updateTransferZoneOverArchingNameIfAbsent(ferryTerminalTransferZone, tags);
      return ferryTerminalTransferZone;
    }
    return null;
  }

  /**
   * process unprocessed ferry terminal that is in Ptv1 tagging that is not part of any stop_area.
   * This assumes the ferry terminal reflects both the transfer zone explicit stop_position on its node
   *
   * @param osmFerryTerminal     to process
   * @param transferZoneType this stop represents
   * @param suppressWarnings when true suppress warning
   * @return created new transfer zone, null otherwise
   */
  private TransferZone processStandAlonePtv1FerryTerminal(
      OsmEntity osmFerryTerminal, TransferZoneType transferZoneType, boolean suppressWarnings) {
    var tags = OsmModelUtil.getTagsAsMap(osmFerryTerminal);

    TransferZone ferryTransferZone = null;
    var defaultMode = OsmWaterModeTags.FERRY;
    var entityType = Osm4JUtils.getEntityType(osmFerryTerminal);
    if(entityType.equals(EntityType.Node)){

      ferryTransferZone = findUserOverwrittenWaitingAreaForFerryTerminal(osmFerryTerminal, tags);
      if(ferryTransferZone != null){
        getTransferConnectoidHelper().extractTransferConnectoidsForMode(
        ...
        boolean terminalOnNetworkNode = hasNetworkLayersWithActiveOsmNode(osmFerryTerminal.getId());
      ...
      throw new PlanItRunTimeException("(PTv2 like) ferry terminal OSM way (%d) encountered while expecting PTV1 " +
          "ferry terminal as node, this shouldn't happen", osmFerryTerminal.getId());
    }
    return ferryTransferZone;
  }

  /**
   * Process remaining ferry terminal that has not been regularly processed due to either non-conformity or not part
   * of a relation.
   * <p>
   *   Note that we have isolated those if they were a platform from regular platform  processing because it behaves
   *   somewhat similar to stations and halts as they may intersect with the ferry infrastructure and need special
   *   treatment to account for various existing combinations and tagging issues that exist
   * </p>
   *
   * @param osmFerryTerminal     to process
   * @param tags of terminal
   * @param suppressWarnings when true suppress warning
   * @return created new transfer zone or attached to existing one successfully, false otherwise
   */
  private TransferZone processStandAlonePtv2LikeFerryTerminal(
      OsmEntity osmFerryTerminal, Map<String, String> tags, boolean suppressWarnings) {

    // OSM way or node representing ferry terminal platform (and/or combined stop position) either tagged or not
    // tagged as such in PTv2. Such a situation would have been delegated from the Ptv1 ferry terminal processing
    // or identified during regular Ptv2 platform processing and postponed to now.
    // this is a platform either with the ferry route attached, or a nearby stop position.
    ...
    if (EntityType.Node.equals(entityType) && hasNetworkLayersWithActiveOsmNode(osmFerryTerminal.getId())) {
      ...
      getTransferConnectoidHelper().createAndRegisterTransferConnectoidsOnTopOfTransferZone(
  ...
  /**
   * Given a ferry stop location and the mode (ferry) to use, construct a new link and link segments attaching the
   * ferry terminal location to the nearest ferry route (within acceptable search radius). We utilise the closest
   * existing ferry link's closest existing node as the
   * attachment point.
   *
   * @param osmFerryStopLocation to use
  ...
  private boolean connectDanglingFerryStopToNearbyFerryRoute(
    ...
    var maxSearchRadius = getSettings().getFerryStopToFerryRouteSearchRadiusMeters();

    /* find closest ferry link to ferry terminal */
    var planitWaterMode = getReferenceNetwork().getModes().get(
        getNetworkToZoningData().getNetworkSettings().getWaterwaySettings().getMappedPlanitWaterMode(osmMode));
    var networkLayer = this.getReferenceNetwork().getLayerByMode(planitWaterMode);
    var boundingBox = getGeoUtils().createBoundingBox(osmFerryStopLocation.getEnvelopeInternal(), maxSearchRadius);
    Collection<MacroscopicLink> spatiallyMatchedLinks =
        getZoningReaderData().getPlanitConverterData().findLinksSpatially(networkLayer, boundingBox);
    ...
    var closestLinkWithDistance = PlanitEntityGeoUtils.findPlanitEntityClosest(
        osmFerryStopLocation.getCoordinate(), spatiallyMatchedLinks, maxSearchRadius, suppressLogging, getGeoUtils());
    ...
    var ferryStopNode = PlanitNetworkLayerUtils.createPopulateAndRegisterNode(
        osmFerryStopLocation, networkLayer, getNetworkToZoningData().getNetworkLayerData(networkLayer));
    ...
    var lineString = PlanitJtsUtils.createLineString(
        osmFerryStopLocation.getCoordinate(), closestNodeWithDistance.first().getPosition().getCoordinate());
    var ferryLink = PlanitNetworkLayerUtils.createPopulateAndRegisterLink(
        ferryStopNode,
        closestNodeWithDistance.first(),
        lineString,
        networkLayer,
        null,
        "dummy-ferry-link",
    ...
    getZoningReaderData().getPlanitConverterData().addLinkToSpatiallyIndexed(networkLayer, ferryLink);

    /* create new ferry link segments */
    String waterWayKey = OsmWaterwayTags.ROUTE;
    String waterWayValue = OsmWaterwayTags.FERRY;
    var linkSegmentType = getReferenceNetwork().getDefaultLinkSegmentTypeByOsmTag(
        waterWayKey, waterWayValue).get(networkLayer);
    var speedLimit =
        getNetworkToZoningData().getNetworkSettings().getWaterwaySettings().getDefaultSpeedLimitByOsmWaterwayType(
            waterWayValue);
    var lanes = getNetworkToZoningData().getNetworkSettings().getDefaultDirectionalLanesByWayType(
        waterWayKey, waterWayValue);
    /* a->b */
  ...
  /**
   * process any remaining unprocessed Ptv1 ferry terminals that are not part of any stop_area.
   * This generally means the ferry terminal reflects both the transfer zone and stop position in a single node.
   * It lacks the public_transport=x key. Hence, we aim for fully constructing both stop position and transfer zone
   * from the single node. It is not allowed to be a way. If it is a way we ignore and treat it as an incomplete Ptv2
   * tagged ferry terminal later when dealing with Ptv2 version of this
   */
  private void processPtv1FerryTerminalsNotPartOfStopArea() {
    OsmZoningReaderOsmData osmData = getZoningReaderData().getOsmConverterData();

    var processedTerminals = new LinkedList<OsmEntity>();
    osmData.getUnprocessedFerryTerminals().forEach(
        (type, unprocessedFerryTerminals) ->
            unprocessedFerryTerminals.forEach( (osmId, unprocessedFerryTerminal) ->{

          if(type.equals(EntityType.Way)){
            // PTV2, or PTV1 with explicit geometry that will be treated as incomplete PTV2 as it
            // is functionally the same
            return;
          }

          var tags = OsmModelUtil.getTagsAsMap(unprocessedFerryTerminal);
          if(OsmPtVersionSchemeUtils.isCompatibleWith(OsmPtVersionScheme.VERSION_2,tags)){
            // explicitly tagged as Ptv2, treat separately because stop position and platform are likely separate
            // entities despite not being part of stop area (which is allowed). Treat in Ptv2 version of
            // ferry terminal parsing
            return;
          }

          /* suppress warnings in case we cannot create a terminal that lies outside the bounding area */
          boolean outsideBoundingArea = !getProjectedBoundingAreaHelper().fallsWithinSpatiallyEligibleBoundingArea(
              unprocessedFerryTerminal, type, tags, getZoningReaderData().getOsmConverterData().getOsmNodeData());
          boolean suppressWarnings = outsideBoundingArea;
    ...
    processedTerminals.forEach( processedFerryTerminal ->
            getZoningReaderData().getOsmConverterData().removeUnprocessedFerryTerminal(processedFerryTerminal));
  }

  /**
   * Process remaining ferry terminals that have not been regularly processed due to non-conformity but are Ptv2 like
   * (have a platform geometry directly adjacent to a route or nearby)
   * <p>
   *   Note that if they were tagged as regular platforms as well they already would have been processed earlier in
   *   a standard way. So either they are not tagged fully or adequately or they represent a stop position of a nearby
   *   platform but are not part of an explicit area
   * </p>
   */
  ...
  private void processFerryTerminalsNotPartOfStopArea() {
    ...
    // combined. If so they are treated as such in the Ptv2(like) handling
    processPtv1FerryTerminalsNotPartOfStopArea();
    processPtv2LikeFerryTerminalsNotPartOfStopArea();

    var remainingTerminals = this.getZoningReaderData().getOsmConverterData().getUnprocessedFerryTerminals();
  ...
  /**
   * Process the stop_position represented by the provided osm node that is nto part of any stop_area and therefore
   * has not been matched to any platform/pole yet, i.e., transfer zone. It is our task to do that now (if possible).
   *
   * @param osmNode          to process as stop_position if possible
   * @param tags             of the node
   * @param suppressWarnings when true suppress warning
   */
  private void processStopPositionNotPartOfStopArea(
      OsmNode osmNode, Map<String, String> tags, boolean suppressWarnings) {
    getZoningReaderData().getOsmConverterData().removeUnprocessedStopPosition(osmNode.getId());
    ...
    for(String osmMode: eligibleOsmModes) {
      ...
      if(!getNetworkToZoningData().getNetworkLayerData(networkLayer).isLocationPresentInLayer(osmNodeLocation)) {
        if(!suppressWarnings){
          LOGGER.fine(String.format("DISCARD: stop_location %d is not part of any parsed link in the network, " +
              "likely incorrectly tagged", osmNode.getId()));
        }
        continue;
      }
      ...
      if(!proceedBecauseIsOverwritten && (matchedTransferZones == null || matchedTransferZones.isEmpty())) {
        /*PTv2 ferry stop tagged as public_transport=stop_position with ferry=yes. It has no terminal nearby
        * to be safe we create one as if this stop position is the terminal as it is likely a tagging error */
        ...
        LOGGER.warning(String.format("DISCARD: stop_position %d has no valid pole, platform, station reference, nor " +
            "close-by infrastructure that qualifies as such for mode %s (tags %s)", osmNode.getId(), osmMode, tags));
        return;
      }
  ...
  /**
   * Process any remaining unprocessed stop_positions that are not part of any stop_area. This means the stop_position
  ...
  private void processStopPositionsNotPartOfStopArea() {
    var unprocessedStopPositions = new TreeMap<>(getZoningReaderData().getOsmConverterData().getUnprocessedStopPositions());
  ...
  /**
   * process a remaining transfer zones without any connectoids that is not part of any stop_area. This means that it
   * has no stop_position or the stop_position has not yet been matched to any platform/pole, i.e., transfer zone.
   * It is our task to do that now (if possible).
   * 
  ...
  private void processIncompleteTransferZone(TransferZone transferZone) {
    ...
    var knownOsmModes = PlanitTransferZoneUtils.getRegisteredOsmModesForTransferZone(transferZone);
    boolean useWaterLeniency = knownOsmModes != null && knownOsmModes.stream().anyMatch(
        m -> OsmWaterModeTags.getSupportedWaterModeTags().contains(m));
    boolean suppressWarning = !getProjectedBoundingAreaHelper().fallsWithinSpatiallyEligibleBoundingArea(
        transferZone,useWaterLeniency);

    /* validate mode support */
    Collection<String> accessOsmModes = OsmModeUtils.extractPublicTransportModesFrom(knownOsmModes);
    if(!getNetworkToZoningData().getNetworkSettings().hasAnyMappedPlanitModeType(accessOsmModes)) {
      if(!suppressWarning) LOGGER.warning(String.format("DISCARD: Waiting area (OSM id %d) has no supported public " +
          "transport PLANit modes present", osmEntityId));
      return;             
    }
        
    /* validate location is separated from infrastructure */
    boolean transferZoneIsNodeOnInfrastructure = osmEntityType.equals(EntityType.Node) &&
        hasNetworkLayersWithActiveOsmNode(osmEntityId);
    if(transferZoneIsNodeOnInfrastructure) {
      // all waiting areas that are a node and are on road/rail/water infrastructure should immediately have been
      // processed including their connectoid creation as they are one and the same, so if we still find one here
      // something went wrong that is worth logging
      // ferries is an exception because waiting area may share node with network but that is not yet converted to
      // the stop position because we tried to find nearby stop position first (and failed given we are now here)
      if (!suppressWarning) LOGGER.severe(String.format("DISCARD: Waiting area node (OSM id %d) on top of " +
              "road/rail/waterway infrastructure did not yet receive connectoids (stop location), this shouldn't " +
              "happen, verify correctness",
          osmEntityId));
      return;
    }     
    
    //per mode - find links, because if the transfer zone supports both a rail and road mode, we require different
    // links for our connectoids
    for(String osmAccessMode : accessOsmModes) {
      ...
      if(getSettings().hasWaitingAreaNominatedOsmWayForStopLocation(osmEntityId, osmEntityType)) {
        ...
        selectedAccessLink = PlanitLinkOsmUtils.getClosestLinkWithOsmWayIdToGeometry(
            osmWayId, transferZone.getGeometry(), networkLayer, getGeoUtils());
        if(selectedAccessLink == null) {
          if(!suppressWarning){
            LOGGER.warning(String.format("DISCARD: User nominated OSM way %d not available for waiting area %s",
                osmWayId, transferZone.getExternalId()));
          }
          return;
        }
      }else {
        ...
        if(OsmWaterModeTags.isWaterModeTag(osmAccessMode) &&
          ...
          boolean success =connectDanglingFerryStopToNearbyFerryRoute(
              connectoinPoint, osmEntityId, OsmWaterwayTags.FERRY, false);
          if(!suppressWarning && success){
            LOGGER.warning(String.format("SALVAGED: Connected dangling Ptv2 ferry platform (%d) to " +
                "waterway network by adding in links", osmEntityId));
          }
        }

        /* regular approach */                     
        Envelope searchBoundingBox = getGeoUtils().createBoundingBox(
            transferZone.getEnvelope(), getSettings().getStopToWaitingAreaSearchRadiusMeters());
        
        /* collect spatially, mode, compatible links */
        Map<MacroscopicNetworkLayer, Collection<MacroscopicLink>> modeSpatiallyCompatibleLinksByLayer =
            findModeBBoxVerticalLayerIdxCompatibleLinksForTransferZone(
                transferZone, osmEntityId, osmAccessMode, searchBoundingBox);
        if(modeSpatiallyCompatibleLinksByLayer == null || modeSpatiallyCompatibleLinksByLayer.isEmpty()) {
          if(!suppressWarning) {
            LOGGER.warning(String.format("DISCARD: No accessible links (max distance %.2fm) for waiting area %s " +
                    "(mode: %s), spatially not available, tagging error, or perhaps nearby road types are deactivated)",
                getSettings().getStopToWaitingAreaSearchRadiusMeters(), transferZone.getExternalId(), osmAccessMode));
          }
          continue;
        }

        var flattenedSpatiallyMatchedLinks = modeSpatiallyCompatibleLinksByLayer.size() == 1 ?
            modeSpatiallyCompatibleLinksByLayer.values().iterator().next() :
            modeSpatiallyCompatibleLinksByLayer.values().stream().flatMap(
                Collection::stream).collect(Collectors.toList());
        
        /* based on candidates, now select the most appropriate option based on a multitude of criteria */
        var accessResult =
            findMostAppropriateStopLocationLinkForWaitingArea(
                transferZone, osmAccessMode, flattenedSpatiallyMatchedLinks);
        selectedAccessLink = accessResult==null ? null : accessResult.first();
      }
       
      /* create connectoids */    
      if(selectedAccessLink != null) {
        getTransferConnectoidHelper().extractTransferConnectoidsForStandAloneTransferZoneByPlanitLink(
  ...
  /**
   * process remaining transfer zones without any connectoids yet that are not part of any stop_area. This means that
   * it has no stop_position or the stop_position
  ...
  private void processIncompleteTransferZones(SortedSet<TransferZone> transferZones) {
    ...
    for(TransferZone transferZone : unprocessedTransferZones) {
      ...
      if(!getZoningReaderData().getPlanitConverterData().getConnectoidData().hasConnectoids(transferZone)) {
  ...
  /**
   * All transfer zones that were created without connectoids AND were found to not be linked to any stop_positions
   * in a stop_area will still have no connectoids. Connectoids need to be created based on implicit stop_position
   * of vehicles which by OSM standards is defined as based on the nearest node. This is what we will do here.
   *
   */
  private void processIncompleteTransferZones() {
    processIncompleteTransferZones(
        getZoningReaderData().getPlanitConverterData().getTransferZonesByOsmId(EntityType.Node));
    processIncompleteTransferZones(
        getZoningReaderData().getPlanitConverterData().getTransferZonesByOsmId(EntityType.Way));
  }
  

   /** Once a station is identified as stand-alone during processing and the transfer zone is created/identified
    * we create its connectoids here.
   * 
   ...
   * @param suppressWarnings when true suppress warnings
   * @param geoUtils to use
   */
  private void extractStandAloneStationConnectoids(
      OsmEntity osmStation,
      Map<String, String> tags,
      TransferZone stationTransferZone,
      SortedSet<String> osmAccessModes,
      boolean suppressWarnings,
    ...
    for(String osmAccessMode : osmAccessModes) {
      var accessModeType =
          getNetworkToZoningData().getNetworkSettings().getMappedPlanitModeType(osmAccessMode);
      MacroscopicNetworkLayer networkLayer = getReferenceNetwork().getLayerByPredefinedModeType(accessModeType);
      
      /* station mode determines where to create stop_locations and how many. It is special in this regard and
      different from a regular transfer zone (platform/pole) */
      Pair<Double, Integer> result =
          determineSearchDistanceAndMaxStopLocationMatchesForStandAloneStation(
              osmStation.getId(), osmAccessMode, getSettings());
      if(result == null) {
        if(!suppressWarnings) {
          LOGGER.warning(String.format("DISCARD: unable to process stand-alone station %d supported mode %s, skip",
              osmStation.getId(), osmAccessMode));
        }
        continue;
      }
      ...
      if(getSettings().hasWaitingAreaNominatedOsmWayForStopLocation(osmStation.getId(), osmStationEntityType)) {
        ...
        long osmWayId = getSettings().getWaitingAreaNominatedOsmWayForStopLocation(
            osmStation.getId(), osmStationEntityType);
        MacroscopicLink nominatedLink =
            PlanitLinkOsmUtils.getClosestLinkWithOsmWayIdToGeometry(
                osmWayId, stationTransferZone.getGeometry(), networkLayer, geoUtils);
        ...
        /* regular approach */
        /* accessible links for station conditioned on found modes, proximity, relative location to transfer zone
        and importance of osm way type (if applicable) */
        Envelope searchBoundingBox = OsmBoundingAreaUtils.createBoundingBoxForOsmEntity(
            osmStation,
            maxSearchDistance ,
            getZoningReaderData().getOsmConverterData().getOsmNodeData().getRegisteredOsmNodes(),
            geoUtils);
      ...
      if(accessLinks == null) {
        if(!suppressWarnings){
          LOGGER.warning(
              String.format("DISCARD: Station %d without eligible access links for pt vehicles as stop locations " +
                  "(tags %s)",osmStation.getId(),  tags));
        }
        return;
      }                   
      ...
      for(var accessLink : accessLinks) {
        ...
        /* identify locations based on links and spatial restrictions, because no stop_location is known in osm,
  ...
  /** Once a station is identified as stand-alone during processing, i.e., no platforms, poles nearby, we must create
   * the appropriate transfer zones (without connectoids). Then afterwards we create the connectoids as well
   * 
   * @param osmStation to extract from
   * @param tags of the station
   * @param suppressWarnings  when true suppress warnings
   * @param geoUtils to use
   */
  private void extractStandAloneStation(
      OsmEntity osmStation, Map<String, String> tags, boolean suppressWarnings, PlanitJtsCrsUtils geoUtils) {
    ...
    TransferZone stationTransferZone;
    EntityType osmStationEntityType = Osm4JUtils.getEntityType(osmStation);
    boolean stationOnTrack = EntityType.Node.equals(osmStationEntityType) &&
        hasNetworkLayersWithActiveOsmNode(osmStation.getId());
    if(stationOnTrack && !getSettings().isOverwriteWaitingAreaOfStopLocation(osmStation.getId())) {
      /* transfer zone + connectoids */
            
      /* station is stop_location as well as transfer zone, create both transfer zone and connectoids based on this
      location */
      OsmNode osmStationNode = getNetworkToZoningData().getNetworkOsmNodes().get(osmStation.getId());
      if(osmStationNode == null){
        if(!suppressWarnings) {
          LOGGER.severe(String.format("DISCARD: Station node (%d) expected to be also a stop location, " +
              "yet OSM node not present on underlying network", osmStation.getId()));
        }
      }
      TransferZoneType ptv1TransferZoneType =
      ...
      we must obtain the transfer zone separately from the connectoids */
      ...
      {        
        if(getSettings().isOverwriteWaitingAreaOfStopLocation(osmStation.getId())) {
          ...
          stationTransferZone =
              getZoningReaderData().getPlanitConverterData().getTransferZoneByOsmId(result.first(), result.second());
          LOGGER.fine(String.format("Mapped station stop_position %d to overwritten waiting area %d",
              osmStation.getId(), result.second()));
          ...
          stationTransferZone =
              getTransferZoneHelper().createAndRegisterTransferZoneWithoutConnectoidsFindAccessModes(
                  osmStation, tags, TransferZoneType.SMALL_STATION, defaultMode);
        }
            
        if(stationTransferZone == null) {
          if(!suppressWarnings){
            LOGGER.warning(String.format("DISCARD: Unable to create transfer zone for OSM station %d",
                osmStation.getId()));
          }
          return;
        }
        PlanitTransferZoneUtils.updateTransferZoneOverArchingNameIfAbsent(stationTransferZone, tags);
      }        
      
      /* if station is assigned a particular pre-existing stop_location, no need to find stop_locations
      (create connectoids) as part of processing the station, this will be taken care of when processing the
      stop_positions, otherwise proceed and construct connectoids directly now */
      // todo: seems weird, would probably be better to postpone the creation of connectoids anyway and mark as
      //  unprocessed?
      if(!getSettings().isWaitingAreaOfStopLocationOverwritten(osmStationEntityType, osmStation.getId())){
        /* connectoids */
        extractStandAloneStationConnectoids(
            osmStation, tags, stationTransferZone, osmAccessModes, suppressWarnings, geoUtils);
      }
  ...
  /**
   * Process the remaining unprocessed Osm entities that were marked for processing, or were identified as not
   * having explicit stop_position, i.e., no connectoids have yet been created.
   * Now that we have successfully parsed all relations (stop_areas) and removed Osm entities part of relations,
   * the remaining entities are guaranteed to not be part of any relation and require
  ...
  private void extractRemainingOsmEntitiesNotPartOfStopArea() throws PlanItException {
    ...
    /* Unproccessed ferry terminal that is not a station -> create transfer zone and connectoids (implicit stop_positions).
     * Only create connectoids when station is on track/road, otherwise flag an error since PTV1 ferry terminals
     * should always be on a route (way)
     */
    processFerryTerminalsNotPartOfStopArea();

    /* unprocessed stop_positions -> do first because ALL transfer zones (without connectoids) are now available
    and we cant try to  match them to these already existing transfer zones if possible */
    processStopPositionsNotPartOfStopArea();    
        
    /* transfer zones without connectoids, i.e., implicit stop_positions not part of any stop_area, -->
    create connectoids for remaining transfer zones without connectoids*/
    processIncompleteTransferZones();                  
  ...
  private void extractKnownPtv2StopAreaStopPosition(
    ...
    if(matchedTransferZones == null || matchedTransferZones.isEmpty()) {
      ...
      if(!suppressLogging) {
        LOGGER.warning(
            String.format("DISCARD: Stop position %d in stop_area %s has no valid pole, platform, " +
                    "station reference, wharf, nor close-by infrastructure that qualifies (tags: %s)",
                osmNode.getId(), transferZoneGroup.getExternalId(), tags.toString()));
      }
      return;
    }             
    ...
    /* connectoids */
    boolean locationIsKnownOsmStopPosition = true;
    getTransferConnectoidHelper().extractTransferConnectoids(
        osmNode, locationIsKnownOsmStopPosition, matchedTransferZones, modeResult.second(),
        transferZoneGroup, suppressLogging);
  }  
  
  /** extract a Ptv2 stop position part of a stop_area relation but not yet identified in the regular phase of
   * parsing as a stop_position. Hence, it is not properly tagged. In this method we try to salvage it by
   * inferring its properties (eligible modes, vertical plane). We do so, by mapping it to the nearest compatible
   * transfer zone available on the stop_area (if any) as a last resort attempt
   * 
  ...
  private void extractUnknownPtv2StopAreaStopPosition(
    ...
    /* not a proper stop_position, so we must infer its properties (eligible modes, transfer zone), eligible OSM
    modes may be null, but may be present */
    ...
    Collection<TransferZone> matchedTransferZones =
        getTransferZoneHelper().findOrCreateTransferZonesForStopPosition(
            osmNode, tags, eligibleOsmModes, transferZoneGroup, suppressLogging);
    suppressLogging = suppressLogging || getSettings().isOverwriteWaitingAreaOfStopLocation(osmNode.getId());
    ...
    if(matchedTransferZones == null || matchedTransferZones.isEmpty()) {
      ...
      if(!suppressLogging && OsmModeUtils.hasMappedPlanitMode(
        ...
        LOGGER.warning(String.format("DISCARD: Stop_position %d without proper tagging on OSM network could not be " +
            "mapped to close-by transfer zone in stop_area (tags: %s)", osmNode.getId(), tags.toString()));
      }      
      return;
    }else if(!suppressLogging && matchedTransferZones.size()>1){
      LOGGER.severe(String.format("" +
          "Identified multiple Spatially closest transfer zones (%s) for stop_position %d that was not tagged as " +
              "such in stop_area %s, this should not happen",
          matchedTransferZones.stream().map(ExternalIdAble::getIdsAsString).collect(Collectors.joining(",")),
          osmNode.getId(), transferZoneGroup.getExternalId()));
    }
    
    TransferZone foundZone = matchedTransferZones.iterator().next();  
    var accessModeTypes = getNetworkToZoningData().getNetworkSettings().getActivatedPlanitModeTypes(
        PlanitTransferZoneUtils.getRegisteredOsmModesForTransferZone(foundZone));
    if(accessModeTypes == null) {
      if(!suppressLogging) LOGGER.warning(String.format(
          "DISCARD: Stop_position %d without proper tagging on OSM network, unable to identify access modes from " +
              "closest transfer zone in stop_area (tags: %s)", osmNode.getId(), tags.toString()));
      return;             
    }
             
    /* connectoids */
    boolean locationIsKnownOsmStopPosition = false;
    boolean success = getTransferConnectoidHelper().extractTransferConnectoids(
        osmNode,
        locationIsKnownOsmStopPosition,
        Collections.singleton(foundZone),
        accessModeTypes,
        transferZoneGroup,
        suppressLogging);
    if(!suppressLogging && success){
      LOGGER.info(String.format("SALVAGED: Stop_position %d in stop_area not marked as such on OSM node, " +
          "mapped to most likely transfer zone (%s) in stop_area instead, verify correctness (tags %s)",
          osmNode.getId(), foundZone.getIdsAsString(), tags.toString()));
    }

  }  
  
  /** extract a Ptv2 stop position part of a stop_area relation. Based on description in
   * <a href="https://wiki.openstreetmap.org/wiki/Tag:public_transport%3Dstop_position">OSM wiki</a>
   *
  ...
  private void extractPtv2StopAreaStopPosition(
    ...
    var osmData = getZoningReaderData().getOsmConverterData();
    ...
    if(member.getType() != EntityType.Node) {
      if(!suppressLogging) {
        LOGGER.severe(String.format("DISCARD: Stop_position %d encountered that it not an OSM node, " +
            "this is not permitted", member.getId()));
      }
      return;
    }      
    ...
    if(stopPositionNode==null) {
      ...
      if(!getZoningReaderData().hasBoundingArea()) {
        LOGGER.warning(String.format("DISCARD: Unable to extract ptv2 stop position %d in OSM relation " +
            "(stop area) %s, OSM node missing", member.getId(), transferZoneGroup.getExternalId()));
      }
      return;
    }

    /* when available but outside of bounding area, we suppress logging since this is likely the main reason for
    any failure downstream  and this it is generally expected to fail, so no point in logging */
    suppressLogging = suppressLogging || !getProjectedBoundingAreaHelper().isPartlyOrWhollyWithinBoundaryArea(
        stopPositionNode, true);
    ...
    /* special cases */
    if(osmData.hasUnprocessedFerryTerminal(EntityType.Node, stopPositionNode.getId())){
      /* special case - a Ptv1 ferry terminal included in Ptv2 stop area where it is not marked as a Ptv2 stop_position
      (albeit with a stop role). If this is the case, attempt to extract a stop position for it (not a transfer zone
      due to its assigned role) */

      /* process as regular stop_position as this is the most likely way we think it was intended */
      // TODO -> it might be that for ferry terminals no platform exists and the intention more often was to create
      //  both platform and stop position. If this turns out to be the case then we should allow for an option
      //  to create transfer zones in this situation
      extractKnownPtv2StopAreaStopPosition(stopPositionNode, tags, transferZoneGroup, suppressLogging);
      /* mark as processed */
      osmData.removeUnprocessedFerryTerminal(stopPositionNode);
    }else {

      /* check if connectoids already exist, if not (due to being part of multiple stop_areas or because it is a
      Ptv1 stop that has already been processed) */
      boolean isPtv2NodeOnly = !OsmPtVersionSchemeUtils.isPtv2StopPositionPtv1Stop(stopPositionNode, tags);
      boolean alreadyProcessed =
          getZoningReaderData().getPlanitConverterData().getConnectoidData().hasAnyTransferConnectoidsForLocation(
            OsmNodeUtils.createPoint(stopPositionNode));
      if(isPtv2NodeOnly && alreadyProcessed && !suppressLogging) {
        /* stop_position resides in multiple stop_areas, this is strongly discouraged by OSM, but does occur
        still, so we identify and skip (no need to process twice anyway)*/
        LOGGER.fine(String.format("Stop_position %d present in multiple stop_areas, discouraged tagging behaviour, " +
            "consider re-tagging",member.getId()));
      }
      if(alreadyProcessed) {
        /* already matched to at least one transfer zone (platform/pole), no need to attempt and create connectoid
        again. In case it serves more than one transfer zone these other transfer zones remain incomplete and will
        be supplemented with the connectoid at a later stage */
        return;
      }

      /* stop-position not processed and not tagged as stop_position known, so it is not properly tagged and we must
      infer mode access from infrastructure it resides on to salvage it */
      if(!suppressLogging) {
        LOGGER.fine(String.format("Stop_position %d in stop_area not marked as such on OSM node, inferring transfer " +
            "zone and access modes by geographically closest transfer zone in stop_area instead ", member.getId()));
      }
  ...
  private void extractPtv2StopAreaPostProcessingEntities(OsmRelation osmRelation) throws PlanItException{
    ...
    TransferZoneGroup transferZoneGroup =
        getZoningReaderData().getPlanitConverterData().getTransferZoneGroupByOsmId(osmRelation.getId());
    if(transferZoneGroup == null) {
      LOGGER.severe(String.format("Found stop_area %d in post-processing for which no PLANit transfer zone group " +
          "has been created, this should not happen",osmRelation.getId()));
    }
    ...
    for(int index = 0 ;index < osmRelation.getNumberOfMembers() ; ++index) {
      ...
      if(member.getRole().equals(OsmPtv2Tags.STOP_ROLE)) {        
        ...
        boolean suppressLogging =
            ...
            getZoningReaderData().getOsmConverterData().isWaitingAreaWithoutMappedPlanitMode(member.getType(), member.getId()) ||
            getZoningReaderData().getOsmConverterData().isIgnoreStopAreaStopPosition(member.getType(), member.getId());
  ...
  /**
   * Wrapped version of handling pr telation in post processing ensureing only spatially eligible and pt compatible
   * entries get processed
   *
   * @param osmRelation to process
  ...
  protected void handleOsmPtRelation(OsmRelation osmRelation, Map<String,String> tags){
    ...
    try {
      ...
      if(OsmPtv2Tags.hasPublicTransportKeyTag(tags) && tags.get(
          OsmPtv2Tags.PUBLIC_TRANSPORT).equals(OsmPtv2Tags.STOP_AREA)) {
      ...
      LOGGER.severe(String.format("Error during parsing of OSM relation (id:%d) for transfer infrastructure",
          osmRelation.getId()));
    }
  }
  ...
  public OsmZoningPostProcessingHandler(
    ...
    super(transferSettings, handlerData, network2ZoningData, profiler);
  }
  ...
  public void initialiseBeforeParsing(){
    ...
    PlanItRunTimeException.throwIf(
        getReferenceNetwork().getTransportLayers() == null ||
            getReferenceNetwork().getTransportLayers().size()<=0,
  ...
  public void complete() throws IOException {
    ...
    try {
      ...
      // below functionality only works on single layer networks for now and assumes that works
   ...
   */
  public void reset() {  
    spatiallyIndexedOsmNodesInternalToPlanitLinks = new HashMap<>();
  }
