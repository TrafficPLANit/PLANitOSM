 ...
 *
 */
public class PlanitOsmTransferZoneParser extends PlanitOsmZoningParserBase{
  ...
  private TransferZone createAndPopulateTransferZone(OsmEntity osmEntity, Map<String, String> tags, TransferZoneType transferZoneType, PlanitJtsCrsUtils geoUtils) throws PlanItException {
    ...
    if(Osm4JUtils.getEntityType(osmEntity).equals(EntityType.Way) && !PlanitOsmWayUtils.isAllOsmWayNodesAvailable((OsmWay)osmEntity, getNetworkToZoningData().getOsmNodes())){
      ...
      if(availableOsmNodeIndex!=null) {
        ...
        if(PlanitOsmBoundingBoxUtils.isNearNetworkBoundingBox(PlanitOsmNodeUtils.createPoint(referenceNode), getNetworkToZoningData().getNetworkBoundingBox(), geoUtils)) {
  ...
  private boolean isTransferZoneOnWrongSideOfRoadOfStopLocation(Point location, TransferZone transferZone, boolean isLeftHandDrive, Mode accessMode, PlanitJtsCrsUtils geoUtils) throws PlanItException {
    ...
    if(planitLinksToCheck!=null){
      Collection<Link> accessibleLinks = PlanitOsmZoningHandlerHelper.excludeLinksOnWrongSideOf(transferZone.getGeometry(), planitLinksToCheck, isLeftHandDrive, Collections.singleton(accessMode), geoUtils);
  ...
  public PlanitOsmTransferZoneParser(
    ...
    this.publicTransportModeParser = new PlanitOsmPublicTransportModeParser(getNetworkToZoningData().getNetworkSettings());
    
    /* parser for identifying pt PLANit modes from OSM entities */
    this.connectoidParser = new PlanitOsmConnectoidParser(zoning, zoningReaderData, transferSettings, profiler);    
  }
   ...
   */   
  public boolean isTransferZoneModeCompatible(TransferZone transferZone, Collection<String> referenceOsmModes, boolean allowPseudoMatches) {
    Collection<String> transferZoneSupportedModes = PlanitOsmTransferZoneParserHelper.getRegisteredOsmModesForTransferZone(transferZone);
  ...
  public TransferZone createAndRegisterTransferZoneWithoutConnectoidsFindAccessModes(
    ...
    }else if(PlanitOsmZoningHandlerHelper.containsMappedPlanitMode(modeResult)){  
      /* mapped planit modes are available and we should create the transfer zone*/
      transferZone = createAndRegisterTransferZoneWithoutConnectoids(osmEntity, tags, transferZoneType, geoUtils);
      PlanitOsmTransferZoneParserHelper.registerOsmModesOnTransferZone(transferZone, modeResult.first());
    }else{
  ...
  public TransferZone createAndRegisterTransferZoneWithoutConnectoidsSetAccessModes(
    ...
    if(transferZone != null) {
      PlanitOsmTransferZoneParserHelper.registerOsmModesOnTransferZone(transferZone, eligibleOsmModes);
    }
    return transferZone;
  }
   ...
   * @throws PlanItException thrown if error
   */
  public Collection<TransferZone> findClosestTransferZonesByTagReference(
      ...
      for(int index=0; index < transferZoneRefValues.length; ++index) {
        ...
        for(TransferZone transferZone : availableTransferZones) {
          ...
          if(refProperty != null && localRefValue.equals(String.class.cast(refProperty))) {
            ...
            if(PlanitOsmTransferZoneParserHelper.getRegisteredOsmModesForTransferZone(transferZone)==null) {
   ...
   */  
  public Collection<TransferZone> findTransferZoneMatchByName(long osmId, String nameToMatch, Collection<TransferZone> availableTransferZones, Collection<String> referenceOsmModes) {
   ...
   * @throws PlanItException thrown if error
   */   
  public Collection<TransferZone> findAccessibleTransferZonesByReferenceOrName(
   ...
   * @throws PlanItException thrown if error
   */
  public Collection<TransferZone> findMostLikelyTransferZonesForStopPositionSpatially(OsmNode osmNode, Map<String, String> tags, Collection<String> referenceOsmModes) throws PlanItException {
